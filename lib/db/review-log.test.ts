...
import { appendReview, listReviewsForTask, toSchedulerReview } from "@/lib/db/review-log";
...
describe("listReviewsForTask", () => {
  it("requests rows ordered by reviewed_at for scheduler rebuild", async () => {
    ...
    const result = await listReviewsForTask("task-1", client);
