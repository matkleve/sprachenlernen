...
export function WordsBisect({
  ...
  return (
    <ShellPageContent width="wide">
      <p className="max-w-2xl text-base leading-relaxed text-muted">{bisectCopy.wordsMinimal}</p>

      {level >= 1 ? (
        <section className="mt-page-content rounded-card border border-line bg-surface-raised p-6 shadow-soft">
        ...
        <section className="mt-page-content">
          ...
          <dl className="mt-6 grid gap-4 sm:grid-cols-3">
            {(
              ...
              <div
                key={item.key}
                className={cn("rounded-card border p-4 shadow-soft", countCardStyles[item.key])}
              >
