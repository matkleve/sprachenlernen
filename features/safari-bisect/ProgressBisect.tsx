...
import {
  ...
  skillNames,
  statusNames,
} from "@/features/progress/content";
import { WeeklyReflectionEntry } from "@/features/progress/weekly-reflection/WeeklyReflectionEntry";

import { copy as bisectCopy } from "./content";

const HABIT_MINUTES_PER_DAY = 15;

type ProgressBisectProps = {
  level: number;
...
export function ProgressBisect({ level, reading, reflection }: ProgressBisectProps) {
  ...
  return (
    <ShellPageContent width="wide">
      <p className="max-w-2xl text-base leading-relaxed text-muted">{bisectCopy.progressMinimal}</p>

      {level >= 1 && reflection.status !== "hidden" ? (
        <WeeklyReflectionEntry reflection={reflection} />
      ) : null}
