 ...
 *
 * Same `ShellHeaderBar` chrome as the signed-in shell — brand chip left,
 * centred title, icon auth controls right on desktop.
 */
export function PublicHeader({ signedIn }: PublicHeaderProps) {
  ...
  const desktopCenter = (
    <p className="max-w-full truncate text-center text-sm font-semibold tracking-tight text-ink">
      {t("header.brand")}
    </p>
  );
