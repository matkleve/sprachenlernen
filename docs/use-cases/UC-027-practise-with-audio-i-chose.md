...
- The learner adds their own sources: an RSS feed, a file, a link. **No
  catalogue** — the app never curates a library of its own.
- Every added item shows its coverage for this learner before it is opened.
- Where an item is too hard as a whole, the app offers the **passages within it**
  that are not. Coverage is computed over a sliding window, not over the episode.
- Items from one series or one speaker are grouped and suggested together, so
  vocabulary repeats and difficulty drops without the material getting easier.
...
- An item with no transcript states plainly that coverage, dictation and
  sentence-level commands are unavailable for it — rather than silently offering
  a degraded version.
- Words the learner replayed repeatedly are offered as cards afterwards.

## Out of scope

A curated podcast catalogue, licensed content, recommendations from other
learners, and generating a transcript where none exists (later).
