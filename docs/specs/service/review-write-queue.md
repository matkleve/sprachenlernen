...
| 3 | Flush fails (offline, 5xx, auth) | Row stays in the queue; a **small, non-blocking** syncing line appears after ~500 ms (see below); retries on reconnect and on a timer — **no manual retry button** |
| 4 | Closes the tab with pending rows | Queue survives in IndexedDB; flush resumes on next visit |
| 5 | Session ends with pending rows | Summary still shows; status reads "Syncing N reviews…" until the queue is empty |
...
- **Pending > 0 for longer than ~500 ms:** one line below the progress counter,
  ...
  the grade buttons. Count includes rows in `failed` state (still queued for
  automatic retry).
- **No separate error + Retry control** on the review surface — the queue resets
  failed rows on each scheduled retry; the learner keeps grading.
