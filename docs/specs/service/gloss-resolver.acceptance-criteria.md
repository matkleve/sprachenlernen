...
- [x] Given `spoken_language` `de` and a published German row for
...
- [x] Given `spoken_language` `de` and no German row, when the back renders,
      then the learner sees the English source gloss — not `card.it:fare…` as
      visible text.
- [x] Given the same `taskId` before and after a spoken-language switch, when
      only `spoken_language` changes, then scheduler state and `taskId` are
      identical.
- [x] Given a content-gap list for a German speaker, when glosses render, then
