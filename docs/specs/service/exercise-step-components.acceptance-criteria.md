...
- [ ] Given a shipped component id in a recipe, when the runner renders that
      step, then the registry delegate renders that component — not a generic
      fallback.
- [ ] Given an unknown `component` id, when the runner renders, then honest
      not-built copy appears — not a silent empty step.
- [ ] Given `gap-fill` under listening defer (UC-077), when the step renders,
