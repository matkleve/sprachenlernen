...
- **In:** `lib/session-sampling.ts` (weights, sample without replacement),
  ...
  today's grades per task). Form-recall **soft staging** weights live here, not
  in a hard gate. Optional per-card `samplingReason` for G1 copy (UC-005).
...
| 2 | Each candidate task | Computes weight `wᵢ = uᵢ × bᵢ × nᵢ × fᵢ` (supplement); `u` from FSRS retrievability |
