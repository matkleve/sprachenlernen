...
the horizontal wood-grain layers — deep macro valleys along the fibre and micro
noise on top — before copying values into `wood-textures.css` or progression
skins.

Complements `/dev/wood-textures` (four fixed species) by exposing the knobs
behind those recipes. No lighting, bevel, or vertical banding — graining only
(STUDY-029 macro + micro stack). Macro must use `180deg`, not `90deg`.

## Scope

- **In:** live preview swatch; sliders for macro **horizontal valleys** and
  micro fibres; four presets; copyable CSS snippet; helpers in
  `lib/grain-creator.ts`.
...
- [ ] Given the raw-planks preset, when selected, then `valleyDarkOpacity`
      exceeds the stock-bar preset's value.
- [ ] Given any control change, when the preview re-renders, then the macro
      layer uses `180deg` (horizontal valleys) and fibre uses anisotropic
      turbulence (`freqY` > `freqX`).
- [ ] Given copy CSS, when clicked, then the clipboard contains a
      `repeating-linear-gradient(180deg` fragment and a `feTurbulence` data-URI.
