...
- **In:** **article layout** with a **section-graphic hero** (image only, no
  title on the image); in-page `<h1>` with full `name` below the hero; summary;
  **badge band** — skill tier icons (bronze+ only, left) and effort chip (right,
  not full-width); **Practical** (effort sentence, single duration chip,
  requirement chips, hosted); **Trains** prose (no redundant skill line when
  ...
  header keeps the methods list title on drill-in (not the method name; shell
  uses `<p>` so the in-page `<h1>` is unique). Desktop back link preserves
  filters; mobile shell back chip.
...
1. Section-graphic hero (no title on image)
2. In-page `<h1>` (`name`)
3. Summary
4. Badge band (skill tier icons + effort)
5. Practical
6. Trains
7. What it does not do (callout)
8. Research confidence (`<details>`)
9. Session footer + Start when applicable
...
- [ ] Given a shipped method, when detail renders, then the hero is image-only,
      the in-page `<h1>` shows the full `name` below it, and summary, Practical,
      Trains, and `doesNotDo` appear below.
