...
Signed-in surface at `/account` for downloading learning history and deleting
...
- **In:** `/account` under `(app)`; export scope `Select` (complete archive vs
...
| --- | --- | --- |
| 1 | Opens `/account` signed in | Export and delete sections render |
...
| 6 | Opens `/account` signed out | Redirect to `/login` (shell gate) |
...
- [ ] Given a signed-in learner on `/account`, when they choose **Review log
...
- [ ] Given `/account`, then axe-core reports no violations.
