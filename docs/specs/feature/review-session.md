...
| 2b | Sees a form-recall card | Spoken-language gloss + cell label; **Type**, **Build**, or **Say it** answer routes; spoken route self-compares then offers **three** grades (no Easy); typed/build check or reveal before grading |
...
use **`flex-1`** so the strip spans the full card width edge to edge. The numeric
`progress` line is unchanged. Owner decision 2026-08-18: strip at top,
