...
- **In:** `GET /api/app-version` returning the deployed Pride version;
  client-side compare against the bundled version; **Reload** control in the
  mobile shell footer (replaces the muted version label while stale); checks on
  mount, `visibilitychange`, and a five-minute interval; `location.reload()` on
  tap.
...
| 3 | Server version is newer | **Reload** ghost button in the same footer slot; version label hidden |
...
| `stale` | server version ≠ bundled version | Reload button | no (until reload) |
...
- [ ] Given the server returns a higher Pride version, when the check completes,
      then a Reload button replaces the version label in the footer slot.
- [ ] Given the learner taps Reload, when the handler runs, then
      `window.location.reload()` is called.
- [ ] Given the tab becomes visible again, when a newer deploy landed since the
      last check, then the Reload control appears without a full navigation.
