...
- [ ] AC-7 · Given band-level adapted text with **personal** coverage ≥ 95 % on
      shown body, when preview renders, then comfortable copy and Start after ~N min.
- [ ] AC-8 · Given band-level adapted text with personal coverage 80–94 %, when
      preview renders, then T1/gap copy appears and Start follows support ladder.
- [ ] AC-10 · Given band-level adapted text with personal coverage &lt; 80 %, when
