...
| **Spoken language** | The language a learner already knows — one value, stored once on the account, used for **both** the app chrome (menus, buttons, grade prompts) and card content (glosses, produce-instructions). Corrected 2026-08-12: originally split into two independent settings ("interface language" / "gloss language"); there is only ever one language the user speaks per account, so one field drives both surfaces. See UC-069. | the learning language, a second independent knob for cards vs chrome |
...
| **Gloss content storage** | Spoken language is one settled field (UC-069) — but *how card gloss text for a non-English spoken language is stored and sourced* is still open: duplicate pool files per spoken language vs. `wordId`-keyed gloss tables, whether a gloss splits into translatable parts, and where non-English glosses come from at all (Kaikki only ships English). See UC-069. |
