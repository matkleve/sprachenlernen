...
| **T-B12** | Scope `poolForScheduling` to the active language only — a live bug, not a gap ([UC-025](use-cases/UC-025-learn-a-second-language.md), corrected 2026-08-12) | `poolForScheduling` (`lib/db/learner-pools.ts`) currently concatenates every learning language's cards into one pool before `buildSession` runs — reachable today by anyone with two learning languages, since `addLearningLanguage` already ships. Fix: scope it exactly like `poolForDisplay` (arguably delete one of the two, since the reason they differed — a cross-language budget — no longer exists), plus a regression test asserting a built session never contains two languages' `taskId` prefixes. `session-builder.md` and `learning-languages.md` are already corrected to state this as the rule; only the code has not caught up |
---
