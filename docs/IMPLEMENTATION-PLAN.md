...
| **2 · Stage-2 display** | Frequency blocks → word detail → pool-local map → T-B3 remainder → forms home + explanations → **session sampling** → **Lernwelt** → per-cell breakdown → form-practice | **T-W20/T-W21 shipped** — next: **T-W22** weighted sampling (UC-079), then **T-W23** Lernwelt service + onboarding, **T-W5** Progress breakdown |
