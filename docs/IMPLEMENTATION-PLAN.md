...
| **T-B5** | Retire the Grundriss worked examples — **docs half done 2026-08-09, code half blocked** | The load-bearing part is finished: `STATE.md` cited item-picker as **the** worked example of state coherence and now cites the review session, which is this product's own and exercises transitions and a terminal state that a selection cannot. The code cannot follow yet: `Select` and `Dialog` have **no consumer outside `features/primitives/`**, so deleting the demo would orphan two shipped primitives and create the boundary-1 violation the task exists to remove. Status table in [`use-cases/README.md`](use-cases/README.md); unblocked by the first feature needing a select or a confirmation (UC-024's delete) |
---
