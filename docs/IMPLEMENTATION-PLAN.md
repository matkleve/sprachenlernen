...
synced 2026-08-20 (T-W22 session sampling shipped).** What to build
...
(`srs-session`), 2000-lemma pools (es + it), **session sampling** (T-W22 /
UC-079), pool-local Progress counts; **forms UX** — T-W20 deck filter, T-W21
form explanations (v0.41.0); **content loop v1** — coverage (`lib/coverage.ts`),
...
reading runner remainder (T-W10); **T-W5** per-cell Progress breakdown; **T-W6**
...
| **2 · Stage-2 display** | Frequency blocks → word detail → pool-local map → T-B3 remainder → forms home + explanations → **session sampling** → per-cell breakdown → form-practice | **T-W22 shipped** — next: **T-W5** Progress breakdown |
