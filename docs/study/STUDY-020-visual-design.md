...
product measured on return ([01](STUDY-001-duolingo.md), D1). Adopting it and omitting
...
didactically right and that nobody opens — an outcome [08](STUDY-008-motivation.md)
...
From [14](STUDY-012-accessibility.md) and
...
[10](STUDY-009-antipatterns.md), A1 is a design rule before it is a product rule. What
is large and at the top gets optimised. So that is where the level profile and
the map go ([19](STUDY-017-milestones-and-map.md)) — not a streak, not an activity
figure.

### G4 · No celebrating error-freeness, no punishing errors

An error is the learning process ([02](STUDY-002-evidence.md), E1). The design must not
...
2. **Serif for what?** Headings and learning text only, or throughout. For
   ...
   the choice is a different one anyway ([18](STUDY-016-language-kit.md), U2).
3. **What does a level that has fallen look like?** The test case for the whole
   ...
   ([03](STUDY-003-level-model.md), honesty rule 1).
