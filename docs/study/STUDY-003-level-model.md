...
nothing to optimise except the language itself (see [01](STUDY-001-duolingo.md), D1).
...
| --- | --- | --- |
| **Vocabulary size** (estimated known lemmas) | SRS holdings, weighted by stability, extrapolated over frequency rank | The only competence quantity that is cheap and reasonably valid to estimate ([02](STUDY-002-evidence.md), E4) |
...
1. **The form→lemma table must store the cell** — `parliamo → (parlare, 1st pl.
   ...
   unit ([18](STUDY-016-language-kit.md), U1).
2. **The paradigm tables in [07](STUDY-007-offline-and-paper.md), Ü3 stop being a side
   exercise** and become training for a measured quantity — and their mixing
   requirement from [02](STUDY-002-evidence.md), E6 now has a target.
...
| B1 | ~2,000–2,750 | ~95 % coverage in **listening** becomes reachable ([02](STUDY-002-evidence.md), E4) |
...
| **not measured** | part of the profile, but no data — never spoke, or every contributing method hidden ([12](STUDY-010-method-cards.md)) | **no** | "not measured", with the route to it |
| **not in profile** | deliberately deselected ([14](STUDY-012-accessibility.md)) | **no** | "not part of your profile" |
...
[14](STUDY-012-accessibility.md) makes profiles of fewer than four skills possible.
...
## What a signal may and may not claim **[D — 2026-08-08, [26](STUDY-024-readiness-and-difficulty.md)]**
...
| **No signal is a requirement for a CEFR band.** | The descriptors were scaled on teachers' judgments of difficulty, not on learner performance ([26](STUDY-024-readiness-and-difficulty.md) R3). No mapping specific enough to support such a claim exists |
| Every statement runs **"this is low because of that"**, never **"do that to get this"** | The second direction publishes the score to feed, which converts this display into a controller — [01](STUDY-001-duolingo.md) D1 with a diagnostic vocabulary |
| Anything shown at paradigm-cell granularity is **frequency-weighted, never counted** | A completion fraction pulls effort toward what is left rather than what pays ([26](STUDY-024-readiness-and-difficulty.md) R8) |
| A signal states the **task type it was measured on** | Receptive evidence licenses no claim about production ([26](STUDY-024-readiness-and-difficulty.md) R9, E3) |
| Nothing rides on a cell-level figure until it survives its own **standard error** | Per-cell mastery is a finer partition of already-thin data, so it is noise for longer than the effect estimate is ([26](STUDY-024-readiness-and-difficulty.md) R6) |
...
of [02](STUDY-002-evidence.md), E7 — it tells you where you stand and demands nothing.
...
- **Cold start.** Where does the first level come from? Proposal: an optional
  ...
  ([01](STUDY-001-duolingo.md), S1). Skipping it starts at A1.1 with a wide uncertainty
  band.
