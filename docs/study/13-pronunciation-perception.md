...
```
  ...
  word not recognised in speech       →  "they talk so fast"
        ↓
  card stays written-only             →  vocabulary with no sound form ([04](04-flashcards-srs.md))
        ↓
...
from unreliable recognition ([01](01-duolingo.md), D4) — is aimed at the wrong
...
([02](02-evidence.md), E1). Three further properties rarely occur together:
...
is expensive and unreliable in [06](06-production.md). It needs:
...
| Connection | How |
| --- | --- |
| **As a method** ([12](12-method-cards.md)) | Its own method card: intensity ●●○, 12 min, headphones, target signal "audio-recall stability for affected words". Floor **[D]** 2×/week while unsolved contrasts exist — zero after |
...
| **To the cards** ([04](04-flashcards-srs.md)) | Words containing an unsolved contrast are systematically disadvantaged in audio recall. After training, exactly those cards should improve — a cleanly checkable effect |
| **To the level** ([03](03-level-model.md)) | Solved contrasts are a layer-1 signal for listening, and an unusually clean one: a genuine ability threshold, not a frequency statistic |
| **To production** ([06](06-production.md)) | Replaces unreliable pronunciation grading with something that demonstrably works. A6 in [10](10-antipatterns.md) says what we do *not* do; this chapter says what we do instead |

---

## Self-comparison as a complement

For the production side [06](06-production.md) stands: your own recording next
