...
> under another name, which is why [03](STUDY-003-level-model.md) measures over cells.
...
[02](STUDY-002-evidence.md) E1, E6 and E13.
...
[25](STUDY-023-why-it-does-not-feel-productive.md) P2, implemented by the product on the
...
[12](STUDY-010-method-cards.md) already accepts this argument for the effect estimate —
...
> [10](STUDY-009-antipatterns.md) A8 already requires.
...
one — [03](STUDY-003-level-model.md) is a display, not a controller. Three dials move on
...
2. **Did it move the one signal it is for.** Each method is judged on a narrow
   target signal, never on the overall level ([12](STUDY-010-method-cards.md)). Narrow
   question, far less noise, answerable in weeks.
3. **Does it work for this person.** Usually not knowable with confidence, for
   the reasons in [12](STUDY-010-method-cards.md). Two guards make it less hopeless: an
