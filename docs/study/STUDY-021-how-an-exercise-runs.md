...
listening with a translation visible ([05](STUDY-005-input-reading-listening.md)) and to
recognition versus recall ([02](STUDY-002-evidence.md), E3): **passing through
...
([21](STUDY-019-method-catalogue-and-context.md)).

### Prepare is where the context model becomes real

[21](STUDY-019-method-catalogue-and-context.md) says a method declares what it needs.
...
- **Only `done` feeds the level model.** Seen is for the interface; done is
  evidence ([03](STUDY-003-level-model.md), level 1).
...
3. **Pausing is allowed and recorded.** Elapsed time matters for the effect
   estimate ([12](STUDY-010-method-cards.md)); a paused timer that silently kept
...
months ([04](STUDY-004-flashcards-srs.md), the leech trap).
...
| Clear when it matters | The **floor** ([12](STUDY-010-method-cards.md)) — stating its reason from the learner's own data, offering a shorter version before accepting no, capped at one prompt a day. "No" ends it |
...
([02](STUDY-002-evidence.md), E3), and it is exactly what the 95–98 % coverage band
quantifies for input ([02](STUDY-002-evidence.md), E4). Below that band, comprehension
...
- Recipe as **data**: ordered steps, so a new method is configuration not a new
  screen ([21](STUDY-019-method-catalogue-and-context.md)).
