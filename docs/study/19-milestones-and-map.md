...
product in [15](15-landscape.md) solves. They all show a **compass** — direction,
...
others only at 4,000–5,000 ([02](02-evidence.md), E4). The difference comes not
...
([18](18-language-kit.md), U1) and **calibrated per language**, not copied from a
table. And the calibration is dated, like everything else
([03](03-level-model.md), rule 4).
...
scheduler, not card count ([04](04-flashcards-srs.md)). Otherwise the bar becomes
an activity metric, and then [10](10-antipatterns.md), A1 applies.
...
against **your** content ([17](17-own-content.md)):
...
([08](08-motivation.md), M5) — with the difference that here it arises
...
This loop is the project's thesis ([15](15-landscape.md), K3). So far it existed
...
- Block boundaries as **language profile data** ([18](18-language-kit.md)), not
  constants in code.
...
- **Sensitive:** K2 compares historical coverage values. Those need a timestamp
  ...
  progress that did not happen ([03](03-level-model.md), rule 4).
