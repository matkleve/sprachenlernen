# 31 · Photographic wood-grain synthesis via 2D FFT

<!-- id: STUDY-031 -->
<!-- type: reasoning -->
<!-- status: active -->
<!-- related: STUDY-030, wood-texture-lab, progression-reference-board -->

## Thesis

For producing wood texture **source material** — reference tiles that seed
`design/progression/` and similar asset folders — the owner's chosen method is
2D-FFT spectral analysis of a real photo, synthesized back into a tile via
`scripts/wood-grain-fourier-synthesis.py`. This is the primary technique for
that job, not a background alternative. It does not replace the live-redrawing
procedural canvas (`lib/wood-grain-ridges.ts`) that `/dev/wood-textures`
depends on for its resize behavior — the two solve different problems: one
produces a baked reference tile, the other renders live at arbitrary sizes.
...
spectrum. Fix: **Heeger–Bergen alternating projection** — repeat (a) force
target spectrum magnitude, keep current phase, (b) histogram-match to the
real defect layer's pixel values. Confirmed by direct comparison: naive
spectrum-only synthesis of an isolated crack layer produced uniform fine
graininess; the same layer after histogram matching produced actual sparse
dark scratch lines, with the histogram plot showing the naive version as a
symmetric bell curve versus the real "spike at zero, long tail" shape.
...
species and has no internal seam to crop away from.
...
- Photo generation prompt: single wood, max resolution, flat/uniform
  ...
  strip out — the owner corrected an earlier "keep it clean" suggestion back
  to keeping real checking and cracking in the source.
