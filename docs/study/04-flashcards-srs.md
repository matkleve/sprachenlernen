...
3. **It supplies the level measurement.** Per-card stability is exactly the
   quantity from which [03](03-level-model.md) computes the difference between
   "seen" and "known".

**Open question:** own implementation or one of the open FSRS libraries. See
[11](11-roadmap-open-questions.md), question 4.
...
Direct connection to [03](03-level-model.md): this picture *is* the vocabulary
...
From [02](02-evidence.md), E3 it follows that there is no single card for a
...
| **Minimal pair** | distinguish *ser* / *estar* | the typical error class ([02](02-evidence.md), E6) | when a confusion is detected |
...
1. **Shipped**, frequency-ordered per language and level — you can start on day
   one without a single decision ([01](01-duolingo.md), S3).
...
3. **Tapped while reading** ([05](05-input-reading-listening.md)) — one tap, one
...
**Solution:** the input side ([05](05-input-reading-listening.md)) preferentially
