...
([11](11-roadmap-open-questions.md), stage 5).

The 20/80 answer is therefore concrete rather than vague:

| Phase | Highest leverage | Why |
| --- | --- | --- |
| Early | Frequency blocks 1–3, listening at coverage | The first 1,000 words buy more comprehension than the next 9,000 ([19](19-milestones-and-map.md)) |
| Middle | 4/3/2, shadowing, form mastery | Fluency comes from repetition and automatisation, not from more words ([20](20-speaking-and-sentences.md)) |
| Later | Free production with correction, avoidance analysis | The remaining gap is what you route around ([06](06-production.md)) |
...
That is the mechanism from [12](12-method-cards.md) doing exactly what it was
built for.
...
[03](03-level-model.md) computes a level from review data; this shows you the
...
([08](08-motivation.md), M5) available on every app open, at zero cost.
...
2. **A measurement.** This is an item with a known difficulty and an observed
   ...
   a test ([01](01-duolingo.md), S1: no exam as a greeting).
...
- The corpus above is **English**. For Spanish and Italian, level-labelled
  ...
  profile and its quality tier ([18](18-language-kit.md)). At tier C the sentence
...
| --- | --- |
| **Steady below-norm accuracy** on a structure | Error rate per paradigm cell ([03](03-level-model.md)) |
...
3. **It falls under the same cap as everything else**: at most one prompt a day,
   ...
   ([12](12-method-cards.md)). Stagnation detection is a floor with a trigger, not
...
([03](03-level-model.md)). Saying that plainly, once, is the single most useful
...
This exposes a structural gap in [21](21-method-catalogue-and-context.md): every
...
ideal self ([16](16-further-findings.md), W4) — *"you could be the person who
...
- **No completion tracking, no streak.** A commitment that gets ticked daily is a
  ...
  [10](10-antipatterns.md), A1 forbids.
...
[10](10-antipatterns.md), A6 — inaccurate feedback confirms errors and damages
