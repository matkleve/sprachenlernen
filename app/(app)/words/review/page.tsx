...
export default async function WordsReviewPage({
  ...
  const { method: methodId, deck: deckParam, minutes: minutesParam } = await searchParams;
  ...
  if (minutesParam) layoutParams.set("minutes", minutesParam);
  ...
  if (!methodId) {
    ...
    const budgetMinutes = method
      ? resolveSessionBudgetMinutes(method.durations, minutesParam)
      : undefined;
    const outcome = await buildSessionAction({ deck, budgetMinutes });
