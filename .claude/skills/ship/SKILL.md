...
- [ ] **Scoped verify green** (`npm run verify:scope`). Run now — do not trust a
      run from earlier in the session.
- [ ] **Full verify** only if cross-cutting or user asked — state the reason. Not
      for merge, `release:shame`, or `release:ship` by default.
...
- [ ] **Learner-facing — three layers** (`docs/AGENT-PITFALLS.md` §21). Report
      ...
      - **Verified:** scoped verify green (paste output). Full only with stated reason.
